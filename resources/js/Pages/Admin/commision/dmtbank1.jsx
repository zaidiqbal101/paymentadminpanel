...
import { Bank1List, UpdateBankCommission } from '@/lib/apis';

export default function Dmtbank1() {
  ...
  useEffect(() => {
    async function fetchBankData() {
      try {
        const response = await Bank1List();
        setBankData(response.data.data || []);
      } catch (error) {
        console.error("Error fetching Bank 1 data:", error);
        setBankData([]);
      }
    }
    fetchBankData();
  ...
  return (
    <div className="max-w-full bg-gray-100 mt-6">
      <div className="bg-gray-700 flex justify-between p-4">
        <h3 className="text-white font-bold">DMT Bank 1 Commission</h3>
      </div>
      ...
      {selectedRecord && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex  z-50">
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-11/12 md:w-2/3 lg:w-2/3 max-h-[100vh] overflow-auto">
