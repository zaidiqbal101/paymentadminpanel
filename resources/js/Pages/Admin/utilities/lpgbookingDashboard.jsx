// components/LPGDashboard.js
import React, { useState, useEffect } from 'react';
import { getLPGdata } from '@/lib/apis';
...
const LPGDashboard = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const result = await getLPGdata();
      console.log('Fetched LPG Data:', result); // Debug: Check the raw data
      setData(result);
      applyFilter(result, filter, startDate, endDate);
    } catch (error) {
      console.error('Failed to fetch LPG data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const applyFilter = (data, filterType, start, end) => {
    ...
    console.log('Filtered Data:', filtered); // Debug: Check the filtered data
  ...
  const successRate =
    totalTransactions > 0
      ? (
          (filteredData.filter(item => item.message.toLowerCase().includes('successful')).length /
            totalTransactions) *
          100
        ).toFixed(2)
      : 0;
  ...
  const successRateTrend = dates.map(date =>
    filteredData
      .filter(item => new Date(item.date_added).toLocaleDateString() === date)
      .reduce((count, item) => count + (item.message.toLowerCase().includes('successful') ? 1 : 0), 0)
  );
  const refundedTrend = dates.map(date =>
    filteredData
      .filter(item => new Date(item.date_added).toLocaleDateString() === date)
      .reduce((sum, item) => sum + parseFloat(item.refunded || 0), 0)
  );
  ...
  const exportData = () => {
    const csv = [
      ['Reference ID', 'Txn ID', 'Operator Name', 'Customer Number', 'Amount', 'TDS', 'Refunded', 'Message', 'Date Added'],
    ...
    a.href = url;
    a.download = 'lpg_transactions.csv';
    a.click();
  ...
  return (
    <div className="container mx-auto p-6 bg-gray-100 min-h-screen">
      <h1 className="text-2xl font-bold mb-4">LPG Dashboard</h1>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white p-4 rounded shadow-md">
              <h3 className="text-lg font-semibold mb-2">Refunded Amount Trend</h3>
              <Line data={refundedChartData} />
            </div>
          </div>
            ...
            <ul>
              {topOperators.map((operator, index) => (
                <li key={index} className="py-2 border-b">
                  {operator.name}: ₹{operator.amount.toFixed(2)}
                </li>
              ))}
            </ul>
          </div>
